...
def upload_file():
    ...
    # ----------------- CUSTOMER SEGMENTATION (Frequency & Monetary) ---------------
    segmentation_data = {}
    if 'CustomerID' in df.columns and 'Sales' in df.columns:
        ...
        fm_df = fm_df.sort_values(by='Frequency', ascending=False).head(10)
        ...
        if len(fm_df) > 1:
            ...
            cluster_labels = kmeans.fit_predict(fm_scaled)
            fm_df['Cluster'] = cluster_labels
            cluster_counts = fm_df['Cluster'].value_counts().to_dict()
        else:
            fm_df['Cluster'] = 0
            cluster_counts = {0: len(fm_df)}

        segmentation_data = {
            'fm_table': fm_df.reset_index().to_dict(orient='records'),
